...
static enum es_result vc_handle_svsm_caa_msr(struct es_em_ctxt *ctxt)
{
	...
	caa = this_cpu_read(svsm_caa);
	regs->ax = caa ? lower_32_bits(__pa(caa)) : 0;
	regs->dx = caa ? upper_32_bits(__pa(caa)) : 0;
 ...
 * the SNP feature we need to ensure that IOMMU supports the SNP feature.
 * The amd_iommu_snp_enable() is used for checking and enabling the feature and,
 * and it is available after subsys_initcall().
 */
